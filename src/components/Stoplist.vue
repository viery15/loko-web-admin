  ...
  <div>
    ...
    <v-client-table
        ...
        <div slot="action" slot-scope="props">
            <button v-on:click="editData(props.row.id_stoplist)" style="margin-left:5px" class="btn btn-md btn-warning btn-sm" data-toggle="modal" data-target="#modal-form"><font-awesome-icon icon="pen"/></button>
            <button v-on:click="destroy(props.row.id_stoplist)" style="margin-left:5px" type="button" class="btn btn-danger btn-sm"><font-awesome-icon icon="trash"/></button>
        </div>
...
export default {
  ...
  data(){
      return {
        ...
        columns: ['stoplist', 'action'],
        optionsTable: {
            headings: {
                stoplist: 'Stoplist',
                action: 'Action',
            },
            sortable: ['stoplist', 'action'],
            filterable: ['stoplist']
        },
        stoplist : '',
        selectedId: '',
  ...
  methods: {
      ...
      newData(){
        ...
        newComponent.append('stoplist', this.stoplist)
      ...
      editData(id){
        $('#btnSave').hide()
        $('#btnUpdate').show()
        this.stoplist = this.dataStoplist.find(x => x.id_stoplist === id).stoplist
        this.selectedId = id
